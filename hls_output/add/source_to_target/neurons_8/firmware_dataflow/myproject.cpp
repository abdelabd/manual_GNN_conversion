...
void myproject(
    ...
    #pragma HLS ARRAY_RESHAPE variable=node_attr cyclic factor=3 dim=1
    #pragma HLS ARRAY_RESHAPE variable=edge_attr cyclic factor=4 dim=1
    #pragma HLS ARRAY_RESHAPE variable=edge_index cyclic factor=2  dim=1
    //#pragma HLS ARRAY_PARTITION variable=layer8_out complete dim=1
    ...
    #pragma HLS ARRAY_PARTITION variable=node_attr_1 cyclic factor=3 dim=1
    #pragma HLS ARRAY_PARTITION variable=node_attr_2 cyclic factor=3 dim=1    
    nnet::replicate_dataflow<input_t,N_NODE,NODE_DIM>(node_attr,node_attr_1,node_attr_2);
    layer4_t layer4_out[N_EDGE*LAYER4_OUT_DIM];
    #pragma HLS ARRAY_PARTITION variable=layer4_out cyclic factor=4 dim=1
    layer4_t layer4_out_1[N_EDGE*LAYER4_OUT_DIM];
    layer4_t layer4_out_2[N_EDGE*LAYER4_OUT_DIM];
    #pragma HLS ARRAY_PARTITION variable=layer4_out_1 cyclic factor=4 dim=1
    #pragma HLS ARRAY_PARTITION variable=layer4_out_2 cyclic factor=4 dim=1
    nnet::edgeblock_dataflow<input2_t, input3_t, layer4_t, config4>(node_attr_1, edge_attr, edge_index, layer4_out, R1_w0, R1_b0, R1_w1, R1_b1, R1_w2, R1_b2, R1_w3, R1_b3); // R1
    nnet::replicate_dataflow<layer4_t,N_EDGE,LAYER4_OUT_DIM>(layer4_out,layer4_out_1,layer4_out_2);
    layer5_t layer5_out[N_NODE*LAYER5_OUT_DIM];
    #pragma HLS ARRAY_PARTITION variable=layer5_out cyclic factor=4 dim=1
    nnet::aggregate_dataflow<input2_t, input3_t, layer5_t, aggregation_config5>(layer4_out_1, edge_index, layer5_out); // aggr5

    layer6_t layer6_out[N_NODE*LAYER6_OUT_DIM];
    #pragma HLS ARRAY_PARTITION variable=layer6_out cyclic factor=3 dim=1
    nnet::nodeblock_dataflow<input_t, layer6_t, config6>(node_attr_2, layer5_out, layer6_out, O_w0, O_b0, O_w1, O_b1, O_w2, O_b2, O_w3, O_b3); // O

    layer7_t layer7_out[N_EDGE*LAYER7_OUT_DIM];
    //#pragma HLS ARRAY_PARTITION variable=layer7_out dim=1
    nnet::edgeblock_dataflow<input2_t, input3_t, layer7_t, config7>(layer6_out, layer4_out_2, edge_index, layer8_out, R2_w0, R2_b0, R2_w1, R2_b1, R2_w2, R2_b2, R2_w3, R2_b3); // R2
