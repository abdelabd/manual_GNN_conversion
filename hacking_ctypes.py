...
# data
indir = "/home/abdel/IRIS_HEP/trackml_data/processed_plus_pyg_small" #(build_heptrkx_plus_pyg.py, pt_min=2, n_phi_sections=2, n_eta_sections=8) 
...
#top_func(Re_1D, Rn_1D, edge_index_1D, hls_pred, ctypes.byref(ctypes.c_ushort()), ctypes.byref(ctypes.c_ushort()), ctypes.byref(ctypes.c_ushort()), ctypes.byref(ctypes.c_ushort()))

#%%
def sigmoid(x):
    return 1/(1+np.exp(-x))

out = sigmoid(hls_pred) #still no good
