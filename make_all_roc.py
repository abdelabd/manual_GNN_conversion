...
def main():
    ...
    # get Brevitas quantization-aware-training (QAT) predictions: 1x(n_graph*n_edge)
    bit_widths = np.arange(2, 20, 2)
    quantized_pred_small = {}
    quantized_pred_large = {}
    for bw in bit_widths:
        # Get quantized model
        quantized_model = InteractionNetwork_quantized(aggr=aggr, flow=flow, hidden_size=n_neurons, bit_width=int(bw))
        quantized_model_dict = os.path.join(config['model']['quantized_state_dict_dir'],
                                            f"train1_PyG_geometric_2GeV_{bw}b.pt")
        try:
        ...
        quantized_pred_small[bw] = [] # small graphs
        for i, data in tqdm(enumerate(graphs_small), total=len(graphs_small)):
            target = data.np_target
            quantized_pred = quantized_model(data).detach().cpu().numpy()
            quantized_pred = reshape_pred(target, quantized_pred)
            quantized_pred_small[bw].append(quantized_pred)
        quantized_pred_small[bw] = np.concatenate(quantized_pred_small[bw])

        quantized_pred_large[bw] = [] # large graphs
        for i, data in tqdm(enumerate(graphs_large), total=len(graphs_large)):
            target = data.np_target
            quantized_pred = quantized_model(data).detach().cpu().numpy()
            quantized_pred = reshape_pred(target, quantized_pred)
            quantized_pred_large[bw].append(quantized_pred)
        quantized_pred_large[bw] = np.concatenate(quantized_pred_large[bw])

    # get HLS4ML post-training-quantization (PQT) predictions: 1x(n_graph*n_edge_max)
    fp_total_bits = np.arange(7, 19, 1)
    precisions = [f"ap_fixed<{X}, 7>" for X in fp_total_bits]
    hls_pred_small = {}
    hls_pred_large = {}
    ...
    # get Brevitas performance on small graphs
    fpr_quantized_small, tpr_quantized_small, auc_quantized_small = {}, {}, {}
    for bw in bit_widths:
        fpr_quantized_small[bw], tpr_quantized_small[bw], _ = roc_curve(target_small, quantized_pred_small[bw])
        auc_quantized_small[bw] = auc(fpr_quantized_small[bw], tpr_quantized_small[bw]) * 100.

    # get Brevitas performance on large graphs
    fpr_quantized_large, tpr_quantized_large, auc_quantized_large = {}, {}, {}
    for bw in bit_widths:
        fpr_quantized_large[bw], tpr_quantized_large[bw], _ = roc_curve(target_large, quantized_pred_large[bw])
        auc_quantized_large[bw] = auc(fpr_quantized_large[bw], tpr_quantized_large[bw]) * 100.
