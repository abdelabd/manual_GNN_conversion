...
def get_events_raw(raw_dir):
    events_raw = np.array(os.listdir(raw_dir))
    events_raw = [i.strip("event00000").strip(".npz") for i in events_raw]
    events_raw = [int(i[:4]) for i in events_raw]
    events_raw = [i for i in set(events_raw)]
    events_raw.sort()
    return events_raw

def get_events_processed(processed_dir):
    events_processed = np.array(os.listdir(processed_dir))
    events_processed = [i.strip("event00000").strip(".npz") for i in events_processed]
    events_processed = [int(i[:4]) for i in events_processed]
    events_processed = [i for i in set(events_processed)]
    events_processed.sort()
    return events_processed

def remove_processed_events(raw_dir, processed_dir):
    prefix = "event00000"
    suffixes = ["-cells.csv", "-hits.csv", "-particles.csv", "-truth.csv"]
    
    events_raw = get_events_raw(raw_dir)
    events_processed = get_events_processed(processed_dir)
    events_to_remove = [i for i in events_processed if i in events_raw]
    
    for e in events_to_remove:
        for suffix in suffixes:
            fullname = prefix + "%s" %e + suffix 
            fullpath = os.path.join(raw_dir, fullname)
            os.remove(fullpath)
    
    events_raw = get_events_raw(raw_dir)
    return len(events_raw), len(events_processed)
        
def main():
    n_raw_left = 1770
    
    while n_raw_left>1:
        raw_dir, processed_dir = top()
        n_raw_left, n_processed_left = remove_processed_events(raw_dir, processed_dir)
        ...
        print(" ")
        print(" ")
        print(f"Events processed: {n_processed_left}")
        print(f"Events remaining: {n_raw_left}")
        print(" ")
        print(" ")
        print(" ")
        print(" ")
